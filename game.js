...
// --- Entities ---
class Entity{constructor(x,y,w,h){
  this.x=x;
  this.y=y;
  this.w=w;
  this.h=h;
...
class Player extends Entity{
  ...
  update(plats){
    ...
    if(Dash && this.dashT<=0 && this.onGround){
      this.dashT=DASH_T;
      this.vx=DASH*this.face;
      this.vy=-2;
    }
    if(this.dashT>0){ 
      this.dashT--;
    }
    if(A && this.attackCD<=0){
      this.attackT=ATTACK_T;
    ...
    this.x=clamp(this.x,-40,W-this.w+40);
    this.y=clamp(this.y,-500,H-this.h);
  }
}
  hitbox(){ 
    if(this.attackT<=0) {
      return null;} 
      const cx=this.face>0? this.x+this.w-4: this.x-ATTACK_R+4;
    return {x:cx, y:this.y+this.h*0.5-18, w:ATTACK_R, h:36}; 
  }
}

class Spider extends Entity{
  constructor(x,y,spd=1.4,hp=2){
    super(x,y,30,22);
    this.speed=spd;
    this.vy=0;
    this.onGround=false;
  ...
  update(plats,player){
    ...
    const sp=chase?this.speed*1.25:this.speed*0.8;
    this.vx = clamp((this.vx + sp*0.18*dir), -sp, sp);
    
    if(chase&&this.onGround&&chance(0.05))
      this.vy=-10.5;
      this.vy+=G*0.9;
      this.x+=this.vx;
      for(const p of plats){
